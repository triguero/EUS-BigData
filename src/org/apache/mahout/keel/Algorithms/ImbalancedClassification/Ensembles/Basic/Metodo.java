...
public class Metodo {
  ...
  public static double nominalDistance[][][];
    ...
    if (distanceEu == false) {
      ...
      for (i = 0; i < nominalDistance.length; i++) {
        if (Attributes.getInputAttribute(i).getType() == Attribute.NOMINAL) {
          ...
          Math.sqrt( Math.abs((SD / ( (double) realTrain.length)) -
                                (media * media)));
        }
      }
    }
  ...
  protected void normalizar() throws CheckException {
    ...
    for (i = 0; i < training.getNumInstances(); i++) {
      ...
      caja[0]= training.getInstance(i).getOutputNominalValuesInt(0); //Added by Isaak, problems with IS copies.
