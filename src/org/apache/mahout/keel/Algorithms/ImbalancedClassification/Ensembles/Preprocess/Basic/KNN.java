...
public int[][] vecinos = null;
public int[] votosIni = null;

    public KNN(int nvec, double conj[][], double real[][], int nominal[][], boolean nulos[][],
                 double conj2[][], double real2[][], int nominal2[][], boolean nulos2[][], boolean distance)  {
        
        this.precalculaDist2(nvec, conj, real, nominal, nulos, conj2, real2, nominal2, nulos2, distance);
        votosIni = new int[2];
        Arrays.fill(votosIni, 0);
    }
    
    public KNN(int[][] vecinos)  {
	...
	public static int evaluacionKNN2 (int nvec, double conj[][], double real[][], int nominal[][], boolean nulos[][], int clases[], double ejemplo[], double ejReal[], int ejNominal[], boolean ejNulos[], int nClases, boolean distance, int vecinos[]) {
        ...
        public int evaluacionKNN2 (int nvec, HashMap<Integer, Boolean> masc, int ej, int clases[]) {

		  int i;
		  int votos[] = votosIni.clone();
		  int votada, votaciones;

                  int vecEncontrados = 0;
                  int clase = 0;
                  votaciones = -1;
                  votada = -1;
		  for (i=0; i<vecinos[ej].length; i++) {
                        
                      if (masc.containsKey(vecinos[ej][i])) {
                          clase = clases[vecinos[ej][i]];
                          votos[clase]++;
                          if (votos[clase] > votaciones) {
                              votaciones = votos[clase];
                              votada = clase;
                          }
                          vecEncontrados++;
                          if (vecEncontrados >= nvec)
                              break;
                      }
        ...
        public void precalculaDist2 (int nvec, double conj[][], double real[][], int nominal[][], boolean nulos[][],
                 double conj2[][], double real2[][], int nominal2[][], boolean nulos2[][], boolean distance) {

		  int i, j;
                 vecinos = new int[conj.length][conj2.length];
                 double dist;
		  for (i=0; i<conj.length; i++) {
                 Neighbor[] vecinosEj = new Neighbor[conj2.length];
                    for (j=0; j<conj2.length; j++) {
                        dist = distanciaNonStatic(conj[i], real[i], nominal[i], nulos[i], conj2[j], real2[j], nominal2[j], nulos2[j], distance);	
                        if (dist == 0)
                            dist = Double.POSITIVE_INFINITY;
                        vecinosEj[j] = new Neighbor(j, dist);
                    }
                    Arrays.sort(vecinosEj);
                    for (j=0; j<conj2.length; j++) {
                        vecinos[i][j] = vecinosEj[j].getIndex();
                    }
                    
		  }
                  System.out.println("Array vecinos = " + vecinos.length + " x " + vecinos[0].length);
	...
	public static double distancia (double ej1[], double ej1Real[], int ej1Nom[], boolean ej1Nul[], double ej2[], double ej2Real[], int ej2Nom[], boolean ej2Nul[], boolean Euc) {
        ...
        public double distanciaNonStatic (double ej1[], double ej1Real[], int ej1Nom[], boolean ej1Nul[], double ej2[], double ej2Real[], int ej2Nom[], boolean ej2Nul[], boolean Euc) {
